    ...
    *,
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X_all, y_all), start=1):
        ...
        with torch.inference_mode():
            ...
            met = evaluate_metrics(y_true, y_pred)
            met["InferenceTotal(ms)"] = total_ms
            met["InferenceTime(ms_per_sample)"] = per_sample_ms
            met["InferencePerModel(ms_per_sample_per_model)"] = per_model_ms
            all_fold_metrics.append(met)

            print(
                f"PD={met['PD']:.4f} PF={met['PF']:.4f} FIR={met['FIR']:.4f} Bal={met['Balance']:.4f} | "
                f"Ensemble: {met['InferenceTime(ms_per_sample)']:.3f} ms/sample, "
                f"{met['InferenceTotal(ms)']:.3f} ms total | "
                f"Per-model: {met['InferencePerModel(ms_per_sample_per_model)']:.3f} ms/sample/model"
            )
    ...
    save_dir = os.path.join("results_sdp", "MLP_DEEP")
...
if __name__ == "__main__":
    ...
    for csv_file in csv_files:
        print(f"\n===== {csv_file} 데이터셋 실험 시작 =====")
        try:
            run_mlp_deep_ensemble(csv_file)
