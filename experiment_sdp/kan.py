...
def run_final_kan_experiment(
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X_all, y_all), start=1):
        ...
        metrics["InferenceTotal(ms)"] = total_ms  # 전체 테스트셋 총 추론시간 (ms)
        metrics["InferenceTime(ms_per_sample)"] = total_ms / N  # 샘플당 추론시간 (ms)
        all_fold_metrics.append(metrics)

        print(f"PD={metrics['PD']:.4f} PF={metrics['PF']:.4f} "
              f"FIR={metrics['FIR']:.4f} Bal={metrics['Balance']:.4f} | "
              f"Total={metrics['InferenceTotal(ms)']:.3f} ms, "
              f"Per-sample={metrics['InferenceTime(ms_per_sample)'] * 1:.3f} ms")
...
if __name__ == "__main__":
    ...
    for csv_file in csv_files:
        print(f"\n===== {csv_file} 데이터셋 실험 시작 =====")
        try:
            run_final_kan_experiment(csv_file)
